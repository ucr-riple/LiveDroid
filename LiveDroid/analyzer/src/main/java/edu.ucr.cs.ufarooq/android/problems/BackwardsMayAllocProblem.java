...
public class BackwardsMayAllocProblem extends DefaultJimpleIFDSTabulationProblem<AccessPathAnalysisObject, InterproceduralCFG<Unit, SootMethod>> {

    private InterproceduralCFG<Unit, SootMethod> icfg;

    public BackwardsMayAllocProblem(InterproceduralCFG<Unit, SootMethod> icfg) {
        super(icfg);
        this.icfg = icfg;
    }
    ...
    public FlowFunctions<Unit, AccessPathAnalysisObject, SootMethod> createFlowFunctionsFactory() {
        return new FlowFunctions<Unit, AccessPathAnalysisObject, SootMethod>() {
            ...
            public FlowFunction<AccessPathAnalysisObject> getNormalFlowFunction(Unit curr, Unit next) {
                ...
                if (Utils.isLoadInst(curr)) {// todo: we might need to treat specially, ignore and get this info from store
                    /*
                ...
                if (curr instanceof DefinitionStmt) {
                    ...
                    if (isStoreStmt) {
                        System.out.println("Alloc(Store): " + curr);
                        Type type = definitionStmt.getFieldRef().getField().getType();
                        if (!Utils.isPrimitiveType(type)) {// only reference Allocations
                            if (definitionStmt.getRightOp() instanceof Local) {
                                Local allocLocal = (Local) definitionStmt.getRightOp();
                                AccessPath allocAccessPath = Utils.getAccessPathUsingMapping(allocLocal, sootMethod, curr, true, icfg);
                                if (allocAccessPath != null) {
                                    ...
                                    if (local != null) {
                                        ...
                                        System.out.println("Alloc (AccessPath): " + accessPath + " : " + local);
                                        if (accessPath != null) {
                                            ...
                                            System.out.println("Alloc (NewAccessPath): " + newAccessPath + " : " + local);
                ...
                return new FlowFunction<AccessPathAnalysisObject>() {
                    ...
                    public Set<AccessPathAnalysisObject> computeTargets(AccessPathAnalysisObject source) {
                        Set<AccessPathAnalysisObject> newAllocs = new HashSet<AccessPathAnalysisObject>();
                        if (source != null) {
